...
async def test_running_saving_aggregated_asset(
    ...
    with mock.patch(
        "defi_common.database.services.async_find_all_addresses"
    ) as find_addresses:
        find_addresses.return_value = [model_address]
        with mock.patch(
            "defi_common.database.services.convert_address_model"
        ) as convert_address:
            ...
            with mock.patch(
                "defi_common.database.services.async_save_aggregated_update"
            ) as save:
